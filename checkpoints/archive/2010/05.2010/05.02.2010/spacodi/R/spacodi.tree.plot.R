spacodi.tree.plot <-function(spacodi.object, phy, sig.plot=TRUE, cut.off=0.05, cols=list("white", "gray", "black"), main=TRUE, outfile=NULL, add.id=FALSE, ...){
			...
			if(sig.plot==TRUE) {
				...
				for(x in 1:nrow(base)){
					if(!is.na(match(base$node.ID[x],rand$node.ID))) {
						...
						if((r.row$obs.Bst-r.row$m.exp.Bst) > 0 && r.row$p.value <= cut.off) {
							col.i[x,2]=cols[[2]]
		...
		for(i in 1:ncol(K)){
			...
			if(sig.plot==TRUE) {
				...
				for(x in 1:nrow(base)){
					if(!is.na(match(base$node.ID[x],rand$node.ID))) {
						r.row=rand[which(rand$node.ID==base$node.ID[x]),]
						if((r.row$obs.K-r.row$m.exp.K) > 0 && r.row$p.value <= cut.off) {
							col.i[x,2]=cols[[2]]
