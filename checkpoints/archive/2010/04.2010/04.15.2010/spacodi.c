...
void spacodi(int *np, int *ns, double *sp_plot, double *distmat, int *abundtype,
			  int *Ndclass, double *dclass, double *Ist_out, double *Pst_out, double *Bst_out, double *PIst_out) // Modified by cetp 17-08-09
{ 
	/*
	Olivier - please replace this code with something functional!
	*/

	int NP,NS,abundt,Ndivc;
	int s1,s2,p1,p2,c, **sps;
	...
	for(p1=1;p1<=NP;p1++)for(p2=p1+1;p2<=NP;p2++) {
		Ist[p1][p2]=Ist[p2][p1]=1.- (((1.-DivIijc[p1][p1][0])+(1.-DivIijc[p2][p2][0]))/2.) / (1.-DivIijc[p1][p2][0]);
		Pst[p1][p2]=Pst[p2][p1]=1.- ((DivPijc[p1][p1][-1]+DivPijc[p2][p2][-1])/2.) / DivPijc[p1][p2][-1];
		Bst[p1][p2]=Bst[p2][p1]=1.- ((DivPijc[p1][p1][-1]/(1.-DivIijc[p1][p1][0])+DivPijc[p2][p2][-1]/(1.-DivIijc[p2][p2][0]))/2.) / (DivPijc[p1][p2][-1]/(1.-DivIijc[p1][p2][0]));
		PIst[p1][p2]=PIst[p2][p1]=1.- ((DivPIijc[p1][p1][-1]+DivPIijc[p2][p2][-1])/2.) / DivPIijc[p1][p2][-1];
