...
# find and define plot limits
	...
	if(all.points==TRUE) {
		...
		if(diff(y.lim.exp)>diff(y.lim))warning("Some expected Bsts were excluded from the plot (but were used in the credible-envelope calculation).")
...
# begin plotting
	...
	if(envelope==TRUE) {
		...
		lines(smooth.spline(o.nodes, min.array, df=df),lty=2)
		lines(smooth.spline(o.nodes, max.array, df=df),lty=2)
	}
	if(sig.plot==TRUE) {
